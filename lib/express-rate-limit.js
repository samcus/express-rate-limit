...
function RateLimit(options) {
    ...
    if (typeof options.store.incr !== 'function' || typeof options.store.time !== 'function' || typeof options.store.resetKey !== 'function') {
    ...
    function rateLimit(req, res, next) {
        ...
        options.store.incr(key, function(err, current) {
            ...
            options.store.time(key, function(err, currentTimer) {
                if (err) {
                  return next(err);
                }

                if (options.headers) {
                  // if(moment().unix > current){
                  //   options.store.res
                  // }
                  res.setHeader('X-RateLimit-Reset', currentTimer);
                }
