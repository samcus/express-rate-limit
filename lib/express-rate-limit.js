...
function RateLimit(options) {
    ...
    options.store = options.store || new MemoryStore(options.windowMs, moment().unix() + options.windowMs);
    ...
    function rateLimit(req, res, next) {
        ...
        options.store.incr(key, function(err, current) {
            ...
            options.store.time(key, function(err, current) {
                if (err) {
                  return next(err);
                }

                if (options.headers) {
                  res.setHeader('X-RateLimit-Reset', current);
                }
